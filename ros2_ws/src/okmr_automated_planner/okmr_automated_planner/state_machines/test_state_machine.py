...
class TestStateMachine(BaseStateMachine):
    ...
    def mission_command_callback(self, msg):
        ...
        if msg.command == MissionCommand.START_MISSION:
            if self.is_waiting_for_mission_start():
                ...
                self.ros_node.get_logger().warn("Mission start command received but a mission is already running")
    ...
    def on_enter_waiting_for_mission_start(self):
        ...
        self.add_subscription(MissionCommand, "/mission_command", self.mission_command_callback)
    ...
    def check_subsystem_enable_success(self, future):
        ...
        try:
            ...
            if response.success:
                self.ros_node.get_logger().info(f"Dead reckoning enabled successfully: {response.message}")
                self.enabling_subsystems_done()
            else:
                self.ros_node.get_logger().error(f"Failed to enable dead reckoning: {response.message}")
                self.abort()
    ...
    def on_enter_enabling_subsystems(self):
        """Enable dead reckoning like in okmr_controls/enable_and_control.py"""
    ...
    def on_enter_test_scan(self):
        self.start_current_state_sub_machine(
            success_callback=self.test_scan_done, 
    ...
    def on_enter_barrel_rolling(self):
        ...
        movement_msg.goal_velocity.duration = 360.0 / self.barrel_roll_speed * number_of_rolls
        ...
        if not success:
            self.ros_node.get_logger().error("Failed to send barrel roll movement command")
            self.queued_method = self.abort
    ...
    def on_enter_surfacing(self):
        ...
        if not success:
            self.ros_node.get_logger().error("Failed to send surfacing movement command")
            self.queued_method = self.abort
        pass
