...
class MovementCommandActionClient:
    ...
    def send_movement_command(
        ...
        on_feedback: Optional[Callable[[object], None]] = None,
    ) -> bool:
        """
    ...
    def cancel_movement(
        ...
        on_cancel_rejection: Optional[Callable[[], None]] = None,
    ) -> bool:
        """
    ...
    def _feedback_callback(self, feedback_msg):
        ...
        self.node.get_logger().debug(
            f"percent done: {round(feedback_msg.feedback.completion_percentage, 2)}"
        )
        if self.on_feedback_callback:
            try:
    ...
    def _cancel_response_callback(self, future):
        ...
        if cancel_response.return_code == 0:  # ACCEPT
            ...
            self.node.get_logger().warn(
                f"Movement cancellation failed with code: {cancel_response.return_code}"
            )
        ...
        if self.on_cancel_acceptance_callback:
            try:
                ...
                self.node.get_logger().error(
                    f"Error in cancel acceptance callback: {e}"
                )
