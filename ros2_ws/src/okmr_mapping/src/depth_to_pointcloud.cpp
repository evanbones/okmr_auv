...
class DepthProjectionNode : public rclcpp::Node {
   ...
   private:
    ...
    pcl::PointCloud<pcl::PointXYZRGBL>::Ptr projectDepthImage (
        const okmr_msgs::msg::SemanticDepth& img, bool rainbow_mode);
...
DepthProjectionNode::DepthProjectionNode ()
    ...
    if (use_semantic) {
        semantic_depth_sub_ = this->create_subscription<okmr_msgs::msg::SemanticDepth> (
            "/semantic_depth", 10,
            std::bind (&DepthProjectionNode::semantic_depth_callback, this, std::placeholders::_1));
    } else {
...
pcl::PointCloud<pcl::PointXYZRGBL>::Ptr DepthProjectionNode::projectDepthImage (
    const okmr_msgs::msg::SemanticDepth& img, bool rainbow_mode = false) {
...
void DepthProjectionNode::semantic_depth_callback (
    const okmr_msgs::msg::SemanticDepth::SharedPtr msg) {
    ...
    if (msg->rgb.width != msg->depth.width || msg->rgb.height != msg->depth.height ||
        ...
        RCLCPP_ERROR (this->get_logger (),
                      "Image dimensions do not match: RGB(%dx%d), Depth(%dx%d), Mask(%dx%d)",
