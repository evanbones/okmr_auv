...
struct chunk_cells {
    static const char* chunk (const char* begin, const char* dead_end) {
        ...
        for (const char* i = begin; i < dead_end; i++) {
            if (*i == delimiter && !quote_opened) return i;
...
class fw_iterator {
   ...
   public:
    ...
    fw_iterator (const char* begin, const char* dead_end)
        : begin_ (begin), end_ (chunk_policy::chunk (begin, dead_end)), dead_end_ (dead_end) {}
...
class mmap_source {
   ...
   public:
    explicit mmap_source (const std::string& path) {
#if defined(_WIN32)
        fd_ = CreateFileA (path.c_str (), GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING,
        ...
        if (size_ > 0) {
            data_ =
                static_cast<const char*> (mmap (nullptr, size_, PROT_READ, MAP_PRIVATE, fd_, 0U));
            if (data_ == MAP_FAILED) {
                close (fd_);
                throw std::system_error (errno, std::system_category ());
            }
...
template <class source = mmap_source, class has_header = has_header<true>,
          class delimiter = delimiter<','>, class quote_char = quote_char<'"'>,
...
class parser {
   ...
   public:
    ...
    explicit parser (Args&&... args) : source_ (std::forward<Args> (args)...) {}
    ...
    using cell_iterator =
        detail::fw_iterator<cell, detail::chunk_cells<delimiter::value, quote_char::value>>;
    ...
    class row {
       ...
       public:
        ...
        row (const char* begin, const char* end) : begin_ (begin), end_ (end) {
            // Handle CRLF line endings by adjusting end_
            if (end_ > begin_ && *(end_ - 1) == '\r') --end_;
        }
        ...
        std::array<cell, sizeof...(Indexes)> cells (Indexes... indexes) const {
            ...
            for (const auto cell : *this) {
                if (index++ == *desired_indexes_it) {
                    ...
                    if (++desired_indexes_it == desired_indexes.end ()) return results;
                }
            }
    ...
    row_iterator begin () const {
        ...
        if constexpr (has_header::value) ++it;
        return it;
    }
    ...
    int index_of (std::string_view column_name) const {
        ...
        for (const auto cell : header ()) {
            if (column_name == cell.trimmed ()) return index;
            index++;
        }
