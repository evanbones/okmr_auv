...
double interpolatePWM(double thrust) {
  if (g_thrusts.empty())
    throw std::runtime_error("Thrust table is empty.");

  if (thrust <= g_thrusts.front())
    return g_pwms.front();
  if (thrust >= g_thrusts.back())
    return g_pwms.back();
...
double thrustToPWM(double thrust) { return interpolatePWM(thrust); }
