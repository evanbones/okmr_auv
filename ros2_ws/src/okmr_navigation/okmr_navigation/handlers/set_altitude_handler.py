...
def handle_set_altitude(goal_handle):  # TODO fix to actually do the altitude math
    ...
    goal_pose = _calculate_relative_goal_pose(
        current_pose, command_msg.translation, command_msg.rotation
    )
    ...
    if (
        command_msg.rotation.x == 0.0
        and command_msg.rotation.y == 0.0
        and command_msg.rotation.z == 0.0
    ):
