...
class OnnxSegmentationDetector(ObjectDetectorNode):
    def __init__(self):
        ...
        self.declare_parameter('model_path', 'gate.onnx') #FIXME: idk why this doesnt work as just 'gate.onnx' as liddetector does that and everything works fine
        ...
        self.providers = self.get_parameter('providers').get_parameter_value().string_array_value
    ...
    def preprocess(self, img: np.ndarray) -> Tuple[np.ndarray, float, int, int]:
        ...
        img_padded = cv2.copyMakeBorder(
            img_resized, top, bot, left, right,
            cv2.BORDER_CONSTANT, value=(114, 114, 114)
        )
        
        # BGR→RGB, HWC→CHW, normalize
        img_preprocessed = img_padded[:, :, ::-1].transpose(2, 0, 1).astype(np.float32) / 255.0
        return np.expand_dims(img_preprocessed, 0), scale, left, top
    
    def postprocess_onnx(self, det, coefs, proto, orig_shape: Tuple[int, int], 
                        scale: float, pad_x: int, pad_y: int) -> np.ndarray:
