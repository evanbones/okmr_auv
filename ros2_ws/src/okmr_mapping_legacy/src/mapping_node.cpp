...
#include "okmr_msgs/srv/vg2pc.hpp"
...
rclcpp::Client<okmr_msgs::srv::Vg2pc>::SharedPtr conversion_client;
rclcpp::Client<okmr_msgs::srv::Matching>::SharedPtr matching_client;
    ...
    for (float x = -w / 2; x <= w / 2; x += voxel_resolution) {
        for (float y = -l / 2; y <= l / 2; y += voxel_resolution) {
            for (float z = -h / 2; z <= h / 2; z += voxel_resolution) {
                if (abs (x) >= w / 2 - voxel_resolution * 2 ||
                    abs (y) >= l / 2 - voxel_resolution * 2 ||
                    abs (z) >= h / 2 - voxel_resolution * 2)
                    cloudData.push_back (voxelData (x, y, z, 0, 100, 255, 0, 0));
            }
        }
    }
...
okmr_msgs::srv::Vg2pc::Response sendConversionRequest (okmr_msgs::srv::Vg2pc::Request request) {
    auto request_ptr = std::make_shared<okmr_msgs::srv::Vg2pc::Request> (request);

    while (!matching_client->wait_for_service (1s)) {
        if (!rclcpp::ok ()) {
            RCLCPP_ERROR (rclcpp::get_logger ("rclcpp"),
                          "Interrupted while waiting for the vg2pc service. Exiting.");
        }
        RCLCPP_INFO (rclcpp::get_logger ("rclcpp"),
                     "vg2pc service not available, waiting again...");
    }
...
okmr_msgs::srv::Matching::Response sendMatchingRequest (okmr_msgs::srv::Matching::Request request) {
    auto request_ptr = std::make_shared<okmr_msgs::srv::Matching::Request> (request);

    while (!matching_client->wait_for_service (1s)) {
        if (!rclcpp::ok ()) {
            RCLCPP_ERROR (rclcpp::get_logger ("rclcpp"),
                          "Interrupted while waiting for the matching service. Exiting.");
        }
        RCLCPP_INFO (rclcpp::get_logger ("rclcpp"),
                     "matching service not available, waiting again...");
    }
...
void find_object_callback (const std::shared_ptr<okmr_msgs::srv::FindObject::Request> request,
                           std::shared_ptr<okmr_msgs::srv::FindObject::Response> response) {
    ...
    okmr_msgs::srv::Vg2pc::Request conversion_request;
    ...
    okmr_msgs::srv::Matching::Request matching_request;
...
okmr_msgs::msg::VoxelGrid serializeGrid () {
    std::ostringstream ofile (std::ios::binary);
    Bonxai::Serialize (ofile, grid);
    std::string s = ofile.str ();
...
void decayAllVoxels () {  // finish this
    ...
    auto lambda = [&accessor] (const voxelData& data, const Bonxai::CoordT& coord) {
        ...
        accessor.value (coord)->confidence *=
            0.997;  // decay TODO: turn into a parameter, maybe make it a formula based on time
    };
    grid.forEachCell (lambda);
}
...
void insertDepthImage (const sensor_msgs::msg::PointCloud2 pc) {
    ...
    for (voxelData vd : cloudData) {
        Bonxai::CoordT coord = grid.posToCoord (vd.x, vd.y, vd.z);
        if (accessor.value (coord) !=
            nullptr) {  // if there is somethign already in the current voxel
            if (vd.confidence >= (*accessor.value (coord)).confidence &&
                (*accessor.value (coord)).class_id ==
                    0)  // only change the class if we are more confident in the new recognition
                accessor.setValue (coord, vd);  // Set voxel value to class ID
        } else
            accessor.setValue (coord, vd);  // if there isnt anything in current voxel, insert the
...
int main (int argc, char** argv) {
    ...
    rclcpp::Subscription<sensor_msgs::msg::PointCloud2>::SharedPtr pc_subscription =
        node->create_subscription<sensor_msgs::msg::PointCloud2> ("/pointcloud", 10,
                                                                  &pc_subscription_callback);

    gridPublisher = node->create_publisher<okmr_msgs::msg::VoxelGrid> ("/voxel_grid", 10);

    rclcpp::Service<okmr_msgs::srv::FindObject>::SharedPtr service =
        node->create_service<okmr_msgs::srv::FindObject> ("find_object", &find_object_callback);
    clientNode = rclcpp::Node::make_shared ("_mapping_client");
    conversion_client = clientNode->create_client<okmr_msgs::srv::Vg2pc> ("vg2pc");
    matching_client = clientNode->create_client<okmr_msgs::srv::Matching> ("pointcloud_matching");

    insertBox (0.3, 0.2, 0.25);  // sample box for testing fgr

    okmr_msgs::srv::Vg2pc::Request request;
