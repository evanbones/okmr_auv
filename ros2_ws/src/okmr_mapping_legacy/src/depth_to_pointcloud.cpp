...
void projectDepthImage (const okmr_msgs::msg::ImageWithPose img) {
    ...
    for (int v = 0; v < h; v += 1) {
        for (int u = 0; u < w; u += 1) {
            ...
            int class_id = static_cast<int> (
                label_img.at<cv::Vec2f> (v, u)[0]);  // Extract class from the first channel
            int confidence = static_cast<int> (
                label_img.at<cv::Vec2f> (v, u)[1]);  // Extract confidence from the second channel
            ...
            if (x > MIN_DIST && x < MAX_DIST) {
                float y =
                    -x * ((u - cx) * fx_inv);       // Calculate real world projection of each pixel
...
int main (int argc, char **argv) {
    ...
    rclcpp::Subscription<okmr_msgs::msg::ImageWithPose>::SharedPtr img_subscription =
        node->create_subscription<okmr_msgs::msg::ImageWithPose> ("/semantic_depth_with_pose", 10,
                                                                  &img_subscription_callback);
