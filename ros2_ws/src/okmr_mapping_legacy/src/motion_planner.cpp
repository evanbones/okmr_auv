...
class MotionPlannerNode : public rclcpp::Node {
   public:
    bool inserted = false;
    MotionPlannerNode () : Node ("motion_planner_node"), costmap (0.04) {
        end_pose_subscription = this->create_subscription<okmr_msgs::msg::GoalPose> (
            "/end_goal_pose", 10, std::bind (&MotionPlannerNode::end_pose_callback, this, _1));

        goal_pose_publisher =
            this->create_publisher<okmr_msgs::msg::GoalPose> ("/current_goal_pose", 10);

        costmap_subscription = this->create_subscription<okmr_msgs::msg::VoxelGrid> (
            "/voxel_grid", 10, std::bind (&MotionPlannerNode::costmap_callback, this, _1));

        current_pose_subscription = this->create_subscription<geometry_msgs::msg::PoseStamped> (
            "/pose", 10, std::bind (&MotionPlannerNode::current_pose_callback, this, _1));

        gridPublisher = this->create_publisher<okmr_msgs::msg::VoxelGrid> (
            "/path_grid", 10);  // voxel grid used specifically for visualization of path

        status_service = this->create_service<okmr_msgs::srv::Status> (
            "motion_planner_status", std::bind (&MotionPlannerNode::status_callback, this,
   ...
   private:
    ...
    struct nodeCompare {
        bool operator() (const node& n1, const node& n2) const {
            return (n1.cost + n1.dist) > (n2.cost + n2.dist);
        }
    };

    float dist (node n1, node n2) {
        return pow (pow (n1.x - n2.x, 2) + pow (n1.y - n2.y, 2) + pow (n1.z - n2.z, 2), 1.0 / 2.0);
    }
    ...
    bool isValidNode (voxelData* pointer) {
        ...
        // if((*pointer)[0]==okmr_msgs::msg::Classes::INFLATED)return false;//node is valid to be
        // travelled to if its part of the costmap or if its empty
        return false;
    }
        ...
        for (float x = n.x - radius; x < n.x + radius; x += 2 * costmap.resolution) {
            for (float y = n.y - radius; y < n.y + radius; y += 2 * costmap.resolution) {
                for (float z = n.z - radius; z < n.z + radius; z += 2 * costmap.resolution) {
                    float squaredDist =
                        (x - n.x) * (x - n.x) + (y - n.y) * (y - n.y) + (z - n.z) * (z - n.z);
                    if (squaredDist <= radius * radius) {
                        if (accessor.value (
                                costmap.posToCoord (x, y, z))) {  // if the cell is occupied
                            return true;
                        }
                    }
                }
    ...
    std::vector<node> aStarSearch () {
        ...
        goal.z =
            -currentEndPoseMsg.pose.position.z;  // why does this need to be negative i have no clue
        ...
        while (!openSet.empty () && reachable) {
            ...
            if (current == goal) {
                ...
                // std::string logMessage = "returning with " + std::to_string(result.size()) + "
                // path nodes"; RCLCPP_INFO(this->get_logger(), logMessage.c_str());
                return result;
            }
            openSet.pop ();
            ...
            for (int x = -1; x <= 1; x++) {
                for (int y = -1; y <= 1; y++) {
                    for (int z = -1; z <= 1; z++) {
                        ...
                        Bonxai::CoordT coord =
                            costmap.posToCoord (tempNode.x, tempNode.y, tempNode.z);
            ...
            for (node n : surrounding) {
                ...
                // std::array<int, 2>* value_ptr = accessor.value(costmap.posToCoord(n.x, n.y,
                // n.z));
                if (isDangerousNode (n, accessor))
                    score += 100 * dist (current, n);  // to discourage using dangerous nodes, but
                                                       // allowing it if really needed
                if (score < n.cost) {
                    previous[n] = current;
                    n.cost = score;
                    n.dist = dist (n, goal) * dist (n, goal) * 50;
                    // if(dist(n,goal)>dist(start,goal)*2)//if we are exploring nodes way far out,
                    // terminate to stop infinite looping reachable=false;//this might trigger early
                    // if goal and start are close
    ...
    gpose calculatePath () {
        ...
        if (path.size () > 1) {
            ...
            while (start2EndError (std::vector<node> (path.begin () + current,
                                                      path.begin () + last + 1)) < threshold &&
                   last < path.size () - 1) {
                last++;
            }
            // std::string logMessage = "Error :" +
            // std::to_string(start2EndError(std::vector<node>(path.begin()+current,path.begin()+last)));
            // RCLCPP_INFO(this->get_logger(), logMessage.c_str());
            current = last;
            voxelData vd =
                voxelData (path[current].x, path[current].y, path[current].z, 2, 100, 0, 255, 0);
            accessor.setValue (
                costmap.posToCoord (path[current].x, path[current].y, path[current].z), vd);
            gpose temp = gpose ();
            ...
            temp.pose = currentPoseMsg.pose;  // if already at goal or failure to calculate path,
                                              // then just stay where it is
            result = temp;
        }
        for (node n : path) {
            voxelData vd = voxelData (n.x, n.y, n.z, 1, 100, 255, 0, 0);
