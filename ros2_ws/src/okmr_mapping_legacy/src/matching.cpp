...
void matching_callback (const shared_ptr<okmr_msgs::srv::Matching::Request> request,
    ...
    source.EstimateNormals (open3d::geometry::KDTreeSearchParamHybrid (2 * 0.02, 30));
    target.EstimateNormals (open3d::geometry::KDTreeSearchParamHybrid (2 * 0.02, 30));
    // get features
    auto source_feature = open3d::pipelines::registration::ComputeFPFHFeature (
        source, open3d::geometry::KDTreeSearchParamHybrid (
                    search_radius, max_neighbours));  // search radius, max neighbours
    auto target_feature = open3d::pipelines::registration::ComputeFPFHFeature (
        target, open3d::geometry::KDTreeSearchParamHybrid (search_radius, max_neighbours));
    ...
    open3d::pipelines::registration::RegistrationResult result =
        open3d::pipelines::registration::FastGlobalRegistrationBasedOnFeatureMatching (
            ...
            open3d::pipelines::registration::FastGlobalRegistrationOption (
                division_factor, use_abs_scale, decrease_mu, distance_threshold, max_iterations,
                touple_scale, max_tuples));
...
int main (int argc, char **argv) {
    ...
    rclcpp::Service<okmr_msgs::srv::Matching>::SharedPtr service =
        node->create_service<okmr_msgs::srv::Matching> ("pointcloud_matching", &matching_callback);
